...
filename = input("Informe o nome da imagem original: ")
filedest = input("Informe o nome da imagem destino: ")
...
def esteganografar(img_orig, img_esteg, str_bin):
    ...
    for x, y in pixels(img.size):
        try:
            ...
            rgb = map(lambda cor, bit: cor - (cor % 2) + int(bit), pix[x, y][:3], [next(str_bin)[1] for _ in range(3)])
            pix[x, y] = tuple(rgb)
...
if __name__ == '__main__':    
    # Oculta a mensagem  na imagem filename e
    # salva o resultado como filedest;
    msg_bin = gera_bin(msg)
    esteganografar(filename, filedest, msg_bin)
